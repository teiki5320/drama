import 'dart:convert';
import 'dart:math';

import 'package:flutter/foundation.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:shared_preferences/shared_preferences.dart';

import '../data/romance_templates.dart';
import '../models/romance.dart';
import 'phone_state_provider.dart';

/// État des threads de romance Tinder — toutes les instances actives
/// (max 3 en parallèle) + historique des arcs terminés.
@immutable
class RomanceThreadsState {
  final List<RomanceInstance> instances;
  /// id template → jour du dernier spawn (pour cooldown).
  final Map<String, int> lastSpawnDay;

  const RomanceThreadsState({
    this.instances = const [],
    this.lastSpawnDay = const {},
  });

  RomanceThreadsState copyWith({
    List<RomanceInstance>? instances,
    Map<String, int>? lastSpawnDay,
  }) =>
      RomanceThreadsState(
        instances: instances ?? this.instances,
        lastSpawnDay: lastSpawnDay ?? this.lastSpawnDay,
      );

  /// Instances actives (non terminées).
  List<RomanceInstance> get active =>
      instances.where((i) => !i.ended).toList();

  /// Instances terminées (archive).
  List<RomanceInstance> get archived =>
      instances.where((i) => i.ended).toList();
}

const _kPrefsKey = 'romance_threads_v1';
const _kMaxParallelArcs = 3;

class RomanceThreadsNotifier extends StateNotifier<RomanceThreadsState> {
  RomanceThreadsNotifier() : super(const RomanceThreadsState()) {
    _hydrate();
  }

  final _rng = Random();
  // instanceId → beatIdx → index de variante piochée (stable au re-tick)
  final _textPicks = <String, Map<int, int>>{};
  final _photoPicks = <String, Map<int, int>>{};

  // ─── Persistance ───────────────────────────────────────────────

  Future<void> _hydrate() async {
    final prefs = await SharedPreferences.getInstance();
    final raw = prefs.getString(_kPrefsKey);
    if (raw == null) return;
    try {
      final j = jsonDecode(raw) as Map<String, dynamic>;
      final list = (j['instances'] as List?) ?? [];
      final spawn = (j['lastSpawnDay'] as Map?)?.map(
            (k, v) => MapEntry(k as String, v as int),
          ) ??
          <String, int>{};
      state = state.copyWith(
        instances: list
            .map((e) => RomanceInstance.fromJson(e as Map<String, dynamic>))
            .toList(),
        lastSpawnDay: spawn,
      );
    } catch (_) {
      // silencieux : on repart de zéro si le JSON est cassé
    }
  }

  Future<void> _persist() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(
      _kPrefsKey,
      jsonEncode({
        'instances': state.instances.map((i) => i.toJson()).toList(),
        'lastSpawnDay': state.lastSpawnDay,
      }),
    );
  }

  // ─── Spawn ────────────────────────────────────────────────────

  /// Tente de spawn un template. Retourne l'instance créée ou null si
  /// trop d'arcs actifs / cooldown / pas de template éligible.
  RomanceInstance? spawn({
    required String templateId,
    required int day,
    required int hour,
    required int minute,
  }) {
    if (state.active.length >= _kMaxParallelArcs) return null;
    final template = kRomanceTemplates.firstWhere(
      (t) => t.id == templateId,
      orElse: () => kRomanceTemplates.first,
    );
    // Cooldown : pas le même archétype trop tôt après le précédent
    final last = state.lastSpawnDay[templateId];
    if (last != null && day - last < template.cooldownDays) return null;
    // Min start day respecté ?
    if (day < template.minStartDay) return null;

    final profileIdx = _rng.nextInt(template.profilePool.length);
    final instance = RomanceInstance(
      id: 'rom_${DateTime.now().millisecondsSinceEpoch}_${_rng.nextInt(9999)}',
      templateId: templateId,
      profileIdx: profileIdx,
      startDay: day,
      startHour: hour,
      startMinute: minute,
    );
    state = state.copyWith(
      instances: [...state.instances, instance],
      lastSpawnDay: {...state.lastSpawnDay, templateId: day},
    );
    _persist();
    return instance;
  }

  /// Spawn aléatoire pondéré — pioche un template éligible.
  RomanceInstance? spawnRandom({
    required int day,
    required int hour,
    required int minute,
  }) {
    if (state.active.length >= _kMaxParallelArcs) return null;
    final eligible = kRomanceTemplates.where((t) {
      if (day < t.minStartDay) return false;
      if (t.maxStartDay != null && day > t.maxStartDay!) return false;
      final last = state.lastSpawnDay[t.id];
      if (last != null && day - last < t.cooldownDays) return false;
      return true;
    }).toList();
    if (eligible.isEmpty) return null;
    // Tirage pondéré
    final totalW = eligible.fold<double>(0, (s, t) => s + t.spawnWeight);
    var roll = _rng.nextDouble() * totalW;
    for (final t in eligible) {
      roll -= t.spawnWeight;
      if (roll <= 0) {
        return spawn(
            templateId: t.id, day: day, hour: hour, minute: minute);
      }
    }
    return null;
  }

  // ─── Tick : avance les instances jusqu'à un choix en attente ─

  /// Avance toutes les instances actives en jouant tous les beats dont
  /// le temps est <= (day, hour, minute). S'arrête à un beat de choix
  /// ou à un beat futur.
  void tickAll({
    required int day,
    required int hour,
    required int minute,
  }) {
    var changed = false;
    final newInstances = <RomanceInstance>[];
    for (final inst in state.instances) {
      if (inst.ended || inst.pendingChoiceBeatIdx != null) {
        newInstances.add(inst);
        continue;
      }
      final advanced = _advanceInstance(inst, day, hour, minute);
      if (advanced != inst) changed = true;
      newInstances.add(advanced);
    }
    if (changed) {
      state = state.copyWith(instances: newInstances);
      _persist();
    }
  }

  /// Avance une seule instance. Pure (ne touche pas l'état global).
  RomanceInstance _advanceInstance(
    RomanceInstance inst,
    int day,
    int hour,
    int minute,
  ) {
    final template = _templateOf(inst);
    var cur = inst;
    while (cur.nextBeatIdx < template.beats.length) {
      final beat = template.beats[cur.nextBeatIdx];
      // Branches : si requireBranch absente ou forbidBranch présente, skip
      if (beat.requireBranch != null &&
          !cur.branches.contains(beat.requireBranch)) {
        cur = cur.copyWith(nextBeatIdx: cur.nextBeatIdx + 1);
        continue;
      }
      if (beat.forbidBranch != null &&
          cur.branches.contains(beat.forbidBranch)) {
        cur = cur.copyWith(nextBeatIdx: cur.nextBeatIdx + 1);
        continue;
      }
      // Temps atteint ?
      final (bd, bh, bm) = _beatFireTime(inst, beat);
      final beatTotal = bd * 24 * 60 + bh * 60 + bm;
      final nowTotal = day * 24 * 60 + hour * 60 + minute;
      if (beatTotal > nowTotal) break;
      // Choix : on s'arrête ici en attente de la réponse Shen
      if (beat.type == RomanceBeatType.choice) {
        cur = cur.copyWith(pendingChoiceBeatIdx: cur.nextBeatIdx);
        break;
      }
      // Sinon : on joue le beat
      cur = _playBeat(cur, beat, bd, bh, bm);
      // Branche posée auto ?
      if (beat.setBranch != null) {
        cur = cur.copyWith(branches: {...cur.branches, beat.setBranch!});
      }
      // Fin d'arc ?
      if (beat.endsArc != null) {
        cur = cur.copyWith(ended: true, endingId: beat.endsArc);
        break;
      }
      cur = cur.copyWith(nextBeatIdx: cur.nextBeatIdx + 1);
    }
    return cur;
  }

  /// Ajoute un PlayedMessage au thread selon le type de beat.
  RomanceInstance _playBeat(
    RomanceInstance inst,
    RomanceBeat beat,
    int day,
    int hour,
    int minute,
  ) {
    final time =
        '${hour.toString().padLeft(2, '0')}:${minute.toString().padLeft(2, '0')}';

    String? text;
    String? photoLabel;
    if (beat.textVariants.isNotEmpty) {
      text = _pickVariant(inst.id, inst.nextBeatIdx, beat.textVariants);
    }
    if (beat.photoLabels.isNotEmpty) {
      photoLabel = _pickPhoto(inst.id, inst.nextBeatIdx, beat.photoLabels);
    }

    return inst.copyWith(
      playedMessages: [
        ...inst.playedMessages,
        PlayedMessage(
          fromThem: beat.fromThem,
          day: day,
          time: time,
          type: beat.type,
          text: text,
          photoLabel: photoLabel,
          voiceDurationS: beat.voiceDurationS,
          callDurationS: beat.callDurationS,
        ),
      ],
    );
  }

  // ─── Réponse Shen à un choix en attente ───────────────────────

  void respondToChoice({
    required String instanceId,
    required int choiceIdx,
    required int day,
    required int hour,
    required int minute,
  }) {
    final idx = state.instances.indexWhere((i) => i.id == instanceId);
    if (idx < 0) return;
    var inst = state.instances[idx];
    if (inst.pendingChoiceBeatIdx == null || inst.ended) return;
    final template = _templateOf(inst);
    final beat = template.beats[inst.pendingChoiceBeatIdx!];
    if (choiceIdx < 0 || choiceIdx >= beat.choices.length) return;
    final choice = beat.choices[choiceIdx];

    // Ajoute le reply de Shen si non vide (ghost = pas de message envoyé)
    final newMessages = List<PlayedMessage>.from(inst.playedMessages);
    if (choice.reply.isNotEmpty) {
      final time =
          '${hour.toString().padLeft(2, '0')}:${minute.toString().padLeft(2, '0')}';
      newMessages.add(PlayedMessage(
        fromThem: false,
        day: day,
        time: time,
        type: RomanceBeatType.text,
        text: choice.reply,
        chosenIndex: choiceIdx,
      ));
    }

    inst = inst.copyWith(
      playedMessages: newMessages,
      branches: choice.setBranch != null
          ? {...inst.branches, choice.setBranch!}
          : inst.branches,
      nextBeatIdx: inst.pendingChoiceBeatIdx! + 1,
      clearPendingChoice: true,
      ended: choice.endsArc != null,
      endingId: choice.endsArc,
    );

    // Mood delta sur Shen
    if (choice.moodDelta != 0) {
      _moodDeltaApply?.call(choice.moodDelta);
    }

    final newInstances = List<RomanceInstance>.from(state.instances);
    newInstances[idx] = inst;
    state = state.copyWith(instances: newInstances);
    // Re-tick immédiatement pour faire enchaîner les beats après réponse
    tickAll(day: day, hour: hour, minute: minute);
    _persist();
  }

  /// Unmatch — Shen jette le profil, l'arc se termine.
  void unmatch(String instanceId) {
    final idx = state.instances.indexWhere((i) => i.id == instanceId);
    if (idx < 0) return;
    final inst = state.instances[idx];
    if (inst.ended) return;
    final newInstances = List<RomanceInstance>.from(state.instances);
    newInstances[idx] = inst.copyWith(
      ended: true,
      endingId: 'shen_unmatch',
      clearPendingChoice: true,
    );
    state = state.copyWith(instances: newInstances);
    _persist();
  }

  // ─── Hook mood ────────────────────────────────────────────────

  /// Branchement du delta mood — injecté depuis le provider à l'init.
  void Function(int delta)? _moodDeltaApply;
  void bindMoodDelta(void Function(int delta) fn) {
    _moodDeltaApply = fn;
  }

  // ─── Helpers ──────────────────────────────────────────────────

  RomanceTemplate _templateOf(RomanceInstance inst) {
    return kRomanceTemplates.firstWhere(
      (t) => t.id == inst.templateId,
      orElse: () => kRomanceTemplates.first,
    );
  }

  /// Convertit (dayOffset, atHour, atMinute) en (jour, heure, minute)
  /// absolus. Cas spécial : dayOffset=0 + atHour=0 → "match time + atMinute".
  (int, int, int) _beatFireTime(RomanceInstance i, RomanceBeat b) {
    var day = i.startDay + b.dayOffset;
    var totalMin = b.atHour * 60 + b.atMinute;
    if (b.dayOffset == 0 && b.atHour == 0) {
      totalMin = i.startHour * 60 + i.startMinute + b.atMinute;
    }
    if (totalMin >= 24 * 60) {
      day += totalMin ~/ (24 * 60);
      totalMin = totalMin % (24 * 60);
    }
    return (day, totalMin ~/ 60, totalMin % 60);
  }

  String _pickVariant(String instId, int beatIdx, List<String> variants) {
    final picks = _textPicks.putIfAbsent(instId, () => {});
    final existing = picks[beatIdx];
    if (existing != null && existing < variants.length) {
      return variants[existing];
    }
    final pick = _rng.nextInt(variants.length);
    picks[beatIdx] = pick;
    return variants[pick];
  }

  String _pickPhoto(String instId, int beatIdx, List<String> photos) {
    final picks = _photoPicks.putIfAbsent(instId, () => {});
    final existing = picks[beatIdx];
    if (existing != null && existing < photos.length) {
      return photos[existing];
    }
    final pick = _rng.nextInt(photos.length);
    picks[beatIdx] = pick;
    return photos[pick];
  }

  // ─── API publique pour lire depuis le UI ──────────────────────

  RomanceTemplate templateOf(RomanceInstance inst) => _templateOf(inst);
  RomanceProfile profileOf(RomanceInstance inst) =>
      _templateOf(inst).profilePool[inst.profileIdx];

  /// Pour le panneau de choix : retourne les choix du beat en attente.
  List<RomanceChoice>? pendingChoices(RomanceInstance inst) {
    if (inst.pendingChoiceBeatIdx == null) return null;
    final beat = _templateOf(inst).beats[inst.pendingChoiceBeatIdx!];
    return beat.choices;
  }
}

final romanceThreadsProvider =
    StateNotifierProvider<RomanceThreadsNotifier, RomanceThreadsState>(
  (ref) {
    final notifier = RomanceThreadsNotifier();
    // Branche le delta mood sur phoneStateProvider
    notifier.bindMoodDelta((delta) {
      final cur = ref.read(phoneStateProvider).mood;
      ref
          .read(phoneStateProvider.notifier)
          .setMood((cur + delta).clamp(0, 10));
    });
    return notifier;
  },
);
